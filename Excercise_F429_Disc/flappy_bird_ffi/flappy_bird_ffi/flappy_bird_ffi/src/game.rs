...
impl<T: InputDevice> Game<T> {
 ...
 //display::draw_start_screen();
    ...
    pub fn draw_game_over_screen() {
        Game::<T>::set_background();
         let text = c"Your score is";
          display::write_string(56, 125, text, color::RED, color::BACKGROUND);
        display::draw_image(80, 160, 40, 80, &assets::GAME_OVER_IMAGE_DATA);
    }

    pub fn draw_start_screen() {
        Game::<T>::set_background();
        display::draw_image(80, 160, 40, 80, &assets::GAME_NAME_IMG_DATA);
        let text = c"Game Starts In";
        display::write_string(48, 120, text, color::RED, color::BACKGROUND);
    }
    ...
    fn run_countdown(&mut self) -> bool {
        ...
        display::write_string(152, 156, number, color::BLACK, color::BACKGROUND);
